...
struct Item //Структура с инфополями и адресным полем
{
	...
	Item<T>* next; //Адресное поле

	Item(T x = 0, Item<T>* n = NULL)
	{
		Data = x;
		next = n;
	}
...
public:
	...
	Item<T>* Search(T x); //поиск по ключу
	void PrintL(); //печать
	T ViewFirst(); //получить значение первого элемента
	void InsertHead(Item<T>*); //вставка в начало
	void InsertEnd(Item<T>*); //вставка в конец
	void InsertPointer(Item<T>*, Item <T>* p); //вставка по ключу

	T ClrHead(); //удаление элемента вначале
	T ClrEnd(); //удаление элемента в конце
	T ClrPointer(Item <T>* p); //удаление элемента по ключу
};
